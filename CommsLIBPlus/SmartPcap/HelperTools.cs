...
namespace CommsLIBPlus.SmartPcap
{
    internal class HelperTools
    {
        ...
        internal static int StringWithLength2Bytes(Span<byte> _buff, string _value)
        {
            if (_buff.Length  < 4 + _value.Length)
                return -1;

            // ASCII so byte count should be equal to chars
            BinaryPrimitives.WriteInt32LittleEndian(_buff, _value.Length);

            // Actual string
            return Encoding.ASCII.GetBytes(_value, _buff.Slice(4)) + 4;
        }
        ...
        internal static uint IP2UInt(string _ip)
        {
            Span<byte> _buffer = stackalloc byte[4];

            ReadOnlySpan<char> ipSpan = _ip;
            uint ipLong = 0;

            for (int i = 0; i < 4; i++)
                ipLong |= uint.Parse(ipSpan.SplitNext('.')) << i * 8;

            return ipLong;
        }

        internal static void WriteUIntIP(Span<byte> span, uint value)
        {
